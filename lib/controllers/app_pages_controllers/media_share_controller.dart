...
class MediaShareController extends GetxController {
  ...
  List mediaList = [appFonts.mediaFile, appFonts.document, appFonts.linkFile];
  ...
  onTapDocs(value) async {
    ...
    final response = await dio.download(
        decryptMessage(value['content']).contains("-BREAK-")
            ? decryptMessage(value!['content']).split("-BREAK-")[1]
            : decryptMessage(value['content']),
        filePath);
  ...
  onTapLinks(value) {
    launchUrl(
        Uri.parse(decryptMessage(value['content']).contains("-BREAK-")
            ? decryptMessage(value!['content']).split("-BREAK-")[0]
            : decryptMessage(value['content'])),
