...
class StatusViewController extends GetxController {
  ...
  int position = -1, selectedIndex = 0, lastPosition = 0;
  Status? status;
  List seenBy = [];
    ...
    if (status!.uid ==
      ...
      FirebaseFirestore.instance
        ...
        if (doc.docs.isNotEmpty) {
          ...
          seenBy = photoUrl[position].seenBy!;
          log("dddd : ${seenBy.length}");
          seenBy = seenBy.where((element){
            log("ELENME ;${element['id']}");
            return element['uid'] != appCtrl.user['id'];
          }).toList();
          update();
          log("ddddsdsd : ${seenBy.length}");
        }
      });
    }
  }
