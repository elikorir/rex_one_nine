...
class LoginController extends GetxController {
  ...
  onTapOtp() async {
    if (mobileGlobalKey.currentState!.validate()) {
      ...
      appCtrl.pref = pref;
      appCtrl.update();
      if (numberController.text == '8612345678') {
        log("GOO :${"${dialCode}8612345678"}");
        await FirebaseFirestore.instance
            .collection(collectionName.users)
            .where("phone", isEqualTo: "${dialCode}8612345678")
            .get()
            .then((value) async {
        ...
        debugPrint(
            "NUMBER : '${dialCode ?? "+44"}${numberController.text.toString()}'");
  ...
  resendCode() async {
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
      phoneNumber: '${dialCode ?? "+44"}${numberController.text.toString()}',
      verificationCompleted: (PhoneAuthCredential credential) {},
      verificationFailed: (FirebaseAuthException e) {},
      codeSent: (String verificationId, int? resendToken) async {
        resendCodeID = verificationId;
    ...
    try {
      ...
      firebaseMessaging.getToken().then((token) async {
        await FirebaseFirestore.instance
          ...
          "statusDesc": "Hello, I am using REX",
