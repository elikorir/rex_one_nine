...
class FetchContactController with ChangeNotifier {
  ...
  onInvitePeople({number}) async {
    ...
    final Uri smsLaunchUri = Uri(
      ...
      query: encodeQueryParameters(<String, String>{
        'body': "Let's chat on Rex! It's a fast, simple, and secure app we can use to message and call our friends for free. https://play.google.com/store/apps/details?id=com.rexapp.chat"
      }),
    );
 ...
 /* searchUser(String search) {
    ...
    String normalizedSearch = search.removeAllWhitespace.toLowerCase();
    ...
    if (registerIndex >= 0) {
      ...
      if (registerIndex >= 0) {
        if (!(searchContactList![phone] == name)) {
          // log("message:::$name");
          searchContactList![phone] = name;
        }
      }
    }
    //
    // // Search in registerContactUser
    // for (var user in registerContactUser) {
    //
    //   if (user.name!.removeAllWhitespace.toLowerCase().contains(normalizedSearch)) {
    //     log("registerContactUser::$registerContactUser");
    //     if (!searchRegisterContactUser.contains(user.name)) {
    //       searchRegisterContactUser.add(user);
    //     }
    //   }
    // }

    // Search in contactList
    contactList?.forEach((phone, name) {
      if (name!.removeAllWhitespace.toLowerCase().contains(normalizedSearch)) {
        searchContactList![phone] = name;
      }
    });

    // Log results for debugging
    log("searchRegisterContactUser: $searchRegisterContactUser");
    log("searchContactList: $searchContactList");
